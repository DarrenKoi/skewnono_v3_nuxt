...
``backend/.env`` via the shared plumbing in ``_runtime/office_redis.py``.
...
``backend/_auth/middleware.py`` does (``row.owner_user_id``, ``row.id``)
...
from backend.api_tokens.providers.mock import (
    _PREFIX,
    _TOUCH_DEBOUNCE,
    _TokenRow,
    _hash,
