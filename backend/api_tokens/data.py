...
path in backend/_auth/middleware.py: a token created via office
...
def _provider():
    if get_data_provider("api_tokens") == "office":
        from backend.api_tokens.providers import office
        return office
    from backend.api_tokens.providers import mock
    return mock
...
def find_by_plaintext(plaintext: str):
    ...
    (backend/_auth/middleware.py::_try_api_token). The mock provider
...
def touch_last_used(token_id: str) -> None:
    ...
    backend/_auth/middleware.py::_try_api_token, right after
