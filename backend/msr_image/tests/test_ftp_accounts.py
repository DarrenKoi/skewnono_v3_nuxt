...
from backend.msr_image import ftp_accounts
from backend.msr_image.config import load_config
from backend.msr_image.contracts import ImageLocator
from backend.msr_image.ftp_accounts import account_for, ftp_account_lookup
from backend.msr_image.providers import office_example as office
from backend.msr_image.tests.test_office_template import FakeFleet
...
def roster(monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.sem_list.data.get_sem_list",
...
def test_a_refresh_failure_serves_the_previous_roster(monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.sem_list.data.get_sem_list", get_sem_list, raising=True
    )
    cfg = load_config({"SKEWNONO_TOOL_FTP_ACCOUNTS": "M16=svc:pw"})
    assert ftp_account_lookup(cfg)("10.0.0.1") == {"user": "svc", "password": "pw"}
...
def test_the_first_load_failure_raises_rather_than_guessing(monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.sem_list.data.get_sem_list", boom, raising=True
    )
    cfg = load_config({"SKEWNONO_TOOL_FTP_ACCOUNTS": "M16=svc:pw"})
...
def test_every_recipe_search_spec_carries_the_tools_account(monkeypatch, roster):
    ...
    from backend.ebeam.recipe_search.providers import office_example as rs
    ...
    monkeypatch.setattr(
        "backend.msr_image.config.load_config", _configured, raising=True
    )
