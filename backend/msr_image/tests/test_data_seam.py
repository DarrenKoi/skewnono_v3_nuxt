...
from backend.msr_image.cache import DiskImageCache, make_cache
from backend.msr_image.config import load_config
from backend.msr_image.errors import ConfigError
...
def test_mock_provider_dispatch(monkeypatch):
    ...
    # Office tools mix JPEG previews with TIFF originals, in varying extension
    # case (.TIF user-confirmed 2026-08-24); the mock mirrors that.
    assert names and all(n.lower().endswith((".jpeg", ".tif")) for n in names)
