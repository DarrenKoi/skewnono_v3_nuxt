...
from backend._runtime import office_redis
...
from backend.ebeam.live_alarm.tests.fake_redis import FakeRedis
...
def fake_package(monkeypatch, tmp_path):
    ...
    ``sys.modules["backend"].__file__``, so the real package would point
    at the real (gitignored, machine-dependent) .env. Returns the directory
    standing in for backend/.
    """
...
def test_client_can_be_built_from_a_dotenv_file_alone(
    ...
    `python -m backend.<feature>.providers.office` — no Flask factory, no
...
def test_repo_root_relative_env_file_wins(monkeypatch, tmp_path, fake_package):
    ...
    (tmp_path / "backend" / ".env").write_text("REDIS_HOST=from-repo-root\n")
...
def test_env_file_beside_the_package_is_the_fallback(
    ...
    monkeypatch.chdir(tmp_path)  # no backend/ here at all
    (fake_package() / ".env").write_text("REDIS_HOST=from-beside-the-package\n")
