...
from backend._runtime.office_template import (
    EDITED,
    MISSING,
    STALE,
    SYNCED,
...
@pytest.fixture
def repo(tmp_path):
    """A repo with backend/sem_list/providers/office_example.py committed."""
    _git(tmp_path, "init", "-q", "-b", "main")
    providers = tmp_path / "backend" / "sem_list" / "providers"
...
def test_a_repo_root_the_template_does_not_live_under_is_not_asked_about(
    ...
    unpacks backend beside (or inside) an unrelated clone hands us a
...
def test_stale_survives_a_template_that_was_moved_to_another_folder(repo):
    ...
    """
    providers = repo / "backend" / "sem_list" / "providers"
    (providers / "office.py").write_text((providers / "office_example.py").read_text())
    before_move = _head_sha(repo)

    moved = repo / "backend" / "ebeam" / "sem_list" / "providers"
    (repo / "backend" / "ebeam").mkdir()
    _git(repo, "mv", "backend/sem_list", "backend/ebeam/sem_list")
    ...
    adapter = office_template.discover(repo / "backend")[0]
