...
from backend._runtime import data_provider, office_registry, site
...
def test_office_without_an_adapter_raises_off_the_app_factory_path(
    ...
    `SKEWNONO_<F>_PROVIDER=office .venv/bin/pytest backend/<f>`, which
    ...
    assert "cp backend/afm/providers/office_example.py" in message
...
def test_validate_env_refuses_office_without_an_adapter(monkeypatch, wired):
    ...
    assert "cp backend/afm/providers/office_example.py" in message
    assert "backend/afm/providers/office.py" in message
...
def test_validate_env_refuses_office_storage_against_a_mock_sem_list(
    ...
    assert "cp backend/sem_list/providers/office_example.py" in message
