...
from backend._runtime import env, site
...
ENV_PY_REL = Path("backend") / "_runtime" / "env.py"


def _relocate(root: Path, prefix: Path):
    """Copy the real env.py to root/backend/_runtime/env.py and load it.
...
def _write_spa(root: Path) -> Path:
    ...
    spa = root / "frontend" / ".output" / "public"
...
def test_the_cloud_prefix_agrees_with_the_bundles_own_preflight():
    ...
    without auth, or the reverse. Imported locally: backend must not
...
def test_env_py_one_level_too_deep_loses_the_spa(tmp_path):
    ...
    the SPA — a package refactor here, or a copy that keeps backend
