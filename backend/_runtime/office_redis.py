...
* :func:`load_env_file` — lazy ``backend/.env`` loading so a standalone
...
def load_env_file(required_var: str = "REDIS_HOST") -> None:
    """Load ``backend/.env`` for standalone / bare-import runs.
    ...
    ``backend`` package — the latter works no matter the current
    ...
    if os.environ.get(required_var):
        return
    pkg = sys.modules.get("backend")  # already imported by any adapter
...
def redis_client() -> redis.Redis:
    ...
    if not host:
        raise RuntimeError(
            "REDIS_HOST is not set. Run from the repo root so backend/.env "
...
def unreachable(detail: str, exc: BaseException) -> RuntimeError:
    ...
    Must be EXACTLY ``RuntimeError``: ``backend/__init__.py`` checks
...
def read_dataframe(raw: bytes, key: str) -> pd.DataFrame:
    ...
    # types exactly (see backend/__init__.py), so an unguarded raise here
