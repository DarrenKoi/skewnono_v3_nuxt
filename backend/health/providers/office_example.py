...
Config (all from `backend/.env`, loaded by the app factory; the shared
...
Self-check at the office, from the repo root:
    ...
    python -m backend.health.providers.office
...
from backend._runtime.office_redis import load_env_file
from backend.health.contracts import ServicesHealthResponse
from backend.health.providers.probe_common import probe_services
...
def _main() -> int:
    """Standalone probe: `python -m backend.health.providers.office`.
