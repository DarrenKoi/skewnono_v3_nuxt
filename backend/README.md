...
2. Create `backend/ebeam/<feature>/` with `__init__.py` (re-exporting
...
3. In `routes.py`, declare `bp = Blueprint("<feature>", __name__)`. Flask
   ...
   `grep -rn "Blueprint(" backend/` first to confirm no collision.
...
4. URL paths inside `routes.py`:
   ...
   - Single-tool: `@bp.get("/<tool>/<feature>/...")` with the tool name baked
     in.
5. The blueprint loader in `backend/__init__.py` discovers any
