...
from backend._auth import admin as admin_mod
from backend._auth import directory as directory_mod
from backend._auth import middleware as middleware_mod
from backend._auth import routes as routes_mod
from backend._auth.directory import Probe
from backend._auth.middleware import install_identity_middleware
from backend._auth.provider import CloudIdentityProvider
...
def test_a_declared_identity_is_not_admin_under_either_phase_default(
    ...
    for cloud in (True, False):
        monkeypatch.setattr(
            "backend._auth.admin.is_cloud", lambda cloud=cloud: cloud
        )
        admin_mod._parse_allowlist.cache_clear()
