...
Run from repo root:  .venv/bin/python -m pytest backend/msr_file
"""
...
from backend.msr_file.contracts import MsrFileResponse
from backend.msr_file.providers import office_example
from backend.msr_file.providers.mock import DYNAMIC_FDC_SPECS
...
def test_office_copy_stays_in_sync_with_template():
    ...
    office = pytest.importorskip(
        "backend.msr_file.providers.office",
...
def test_health_scales_by_the_shared_bad_threshold():
    ...
    조용히 어긋나던 자리입니다.
    """
    from backend.msr_file.providers.mock import FDC_BAD_SIGMA, _fdc_status
