...
* ``minio_pkl`` is a key relative to the configured ``PREFIX`` — NOT
  ...
  and not from .env (see the warning in ``backend/.env.example``).
...
At the office: fill in OPENSEARCH_* in ``backend/.env`` and
...
from backend.msr_file.contracts import (
    AlignmentInfo,
    ExeDetailInfo,
    FdcParamSummary,
    MsrArtifact,
...
from backend.msr_file.providers.mock import (
    DYNAMIC_FDC_SPECS,
    FDC_BAD_SIGMA,
    FDC_CATEGORY_LABELS,
    _fdc_status,
...
if __name__ == "__main__":
    ...
    #     .venv/bin/python -m backend.msr_file.providers.office
