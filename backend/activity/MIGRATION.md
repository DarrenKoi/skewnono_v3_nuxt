...
`backend/_logging/target.py` hands the same target to the writer and the
...
`frontend/app/utils/activity.ts` holds
...
`backend/_logging/activity.py` performs the classification and FAB
...
```bash
cp backend/activity/providers/office_example.py \
  backend/activity/providers/office.py
```
