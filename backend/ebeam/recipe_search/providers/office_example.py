...
``backend/.env`` (self-loaded), and FTP credentials from
...
from backend._core.cond_cursor import CursorMarks, cursor_info_from_cond
from backend._runtime.office_redis import redis_client as _redis_client
from backend.ebeam._office_search import fetch_hits, query, ttl_cache
from backend.ebeam.recipe_search import rawfiles
from backend.msr_image.errors import SourceUnavailable
from backend.ebeam.recipe_search.contracts import (
    AlignDetailResponse,
    AlignImage,
    AlignImagesResponse,
    AlignPoint,
...
from backend.ebeam.recipe_search.providers.mock import (
    IMAGE_SLOTS,
...
def _missing_key_error(key: str) -> LookupError:
    return LookupError(
        ...
        "REDIS_PASSWORD in backend/.env and that the recipe-list job "
...
def _locate_idp(
    ...
    try:
        ...
        if not notes:
            raise
        # Bare LookupError, never a subclass: backend/__init__.py maps
...
def _download_idp(location: _IdpLocation) -> bytes:
    ...
    from backend.msr_image.ftp_accounts import ftp_account_lookup
    from backend.msr_image.paths import validate_tool_ip
...
def _parse_idp(data: bytes, label: str) -> dict[str, pd.DataFrame]:
    ...
    an office PC. Set it in ``backend/.env``, restart, and tell home.
...
def fetch_recipe_image(locator: IdpLocator, name: str) -> tuple[bytes, str]:
    ...
    """
    from backend.msr_image.providers.office_example import _content_type
...
if __name__ == "__main__":
    ...
    #     .venv/bin/python -m backend.ebeam.recipe_search.providers.office
