...
Full context:
[`backend/msr_image/MIGRATION.md`](../../msr_image/MIGRATION.md); the
...
- Office data source: **WIRED — tool FTP, located from the Redis recipe
  ...
  - `eqp_ip` outside `SKEWNONO_TOOL_SUBNETS` → that candidate is skipped with a
    ...
    (`backend/msr_image/errors.py`), and `backend/__init__.py`
...
- Handler: `routes.py` → `data.get_recipe_compare_data(tool_type, recipes)`.
  ...
  `frontend` recipe-search compare UI) as JSON:
...
## Verify
    ...
    SKEWNONO_RECIPE_SEARCH_PROVIDER=office .venv/bin/pytest backend/ebeam/recipe_search
