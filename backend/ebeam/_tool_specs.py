...
`frontend/app/utils/toolType.ts` both apply.
...
`classifyToolType()` in frontend/app/utils/toolType.ts). Never parse
the id itself -- see
...
# `classifyToolType()` in frontend/app/utils/toolType.ts.
...
def model_to_tool_type(eqp_model_cd: str) -> ToolType | None:
    ...
    frontend/app/utils/toolType.ts. The two must agree, and it is
    enforced: both read the same `backend/ebeam/__fixtures__/
    tool_type_cases.json`, via `backend/ebeam/tests/
    test_tool_type_parity.py` on this side and `frontend/app/utils/
