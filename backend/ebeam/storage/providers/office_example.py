...
# gitignored (backend/**/providers/office.py); this office_example.py
...
``REDIS_PASSWORD`` in ``backend/.env`` (same vars as sem_list/health).
...
from backend.ebeam.storage.contracts import (
    PpidUnavailableRow,
...
def get_storage(
    ...
    if raw is None:
        raise LookupError(
            ...
            "REDIS_PASSWORD in backend/.env and that the storage job has "
            "populated the key."
        )
...
if __name__ == "__main__":
    ...
    #     .venv/bin/python -m backend.ebeam.storage.providers.office
    # (_redis_client loads backend/.env itself if the env isn't set.)
