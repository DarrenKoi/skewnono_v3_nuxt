...
- Office data source: per-tool parquet `pandas.DataFrame` in Redis —
  ...
  `REDIS_HOST`/`REDIS_PORT`/`REDIS_PASSWORD` in `backend/.env`.
...
First copy the tracked skeleton to the gitignored runnable adapter:
    ...
    cp backend/ebeam/storage/providers/office_example.py \
...
Contract gate (`.env` loaded by `backend/conftest.py`):
    ...
    SKEWNONO_STORAGE_PROVIDER=office .venv/bin/pytest backend/ebeam/storage
