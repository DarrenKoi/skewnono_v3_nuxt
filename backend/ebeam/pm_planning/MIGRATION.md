...
- This route is slug-parameterized: `routes.py` registers a single
  ...
  `/api` by `backend/__init__.py`'s `routes.py`-rglob loop (there is
  no per-slug blueprint registration — one blueprint handles every slug at
  request time). `tool_slug` must be in
  `backend/ebeam/_tool_specs.py`'s `SEM_TOOL_SLUGS`
...
## Verify
    ...
    SKEWNONO_PM_PLANNING_PROVIDER=office .venv/bin/pytest backend/ebeam/pm_planning
