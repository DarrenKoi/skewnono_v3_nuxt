...
``OPENSEARCH_USER`` / ``OPENSEARCH_PASSWORD`` in ``backend/.env``
...
from backend.ebeam._office_search import (  # noqa: F401
    CACHE_TTL_SECONDS,
    KST,
...
def device_desc() -> dict[str, dict[str, str]]:
    ...
    if raw is None:
        raise LookupError(
            ...
            "backend/.env and that the device catalog is populated."
        )
...
def r3_device_grp() -> dict[str, str]:
    ...
    if raw is None:
        raise LookupError(
            ...
            "backend/.env and that the R3 device catalog is populated."
        )
