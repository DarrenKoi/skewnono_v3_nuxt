...
- OpenSearch plumbing (client, composite walker, lot_id↔lot_cd bridge,
  ...
  `backend/ebeam/_office_meas_hist.py` — recipe_tat's office
...
1. **`FAIL_INDEX_CEIL = 1.25` / `FAIL_INDEX_FLOOR = 0.75`**
   (`frontend/app/utils/failEquipmentSignals.ts`) 이 둘은 **통계
...
## Verify
    ...
    SKEWNONO_FAIL_ISSUE_PROVIDER=office .venv/bin/pytest backend/ebeam/fail_issue
