...
픽스처:      backend/ebeam/recipe_tat/__fixtures__/
...
아닙니다. 칸마다 순번 0(느림) 장비의 speed 만은 이 ±4 % 폭 밖의
U(1.60, 1.75)로 훨씬 크게 벌립니다 — 그 이유(5대짜리 mock 셀이 tat_index를
구조적으로 감쇠시킨다는 것과, 이 값이 사무실 실 데이터에 대한 주장이 아니라
느림/빠름 배지를 홈에서 시연하기 위한 장치라는 것)는 `_tool_scalars`의
독스트링에 자세히 적어뒀습니다.
...
from backend.ebeam.device_statistics.providers.mock import _lot_index
from backend.ebeam._analytics import (
    MeasurementScope,
    fab_base,
    filter_measurements,
    lot_metadata,
...
from backend.ebeam._tool_specs import SEM_TOOL_TYPES, model_to_tool_type
from backend.ebeam.recipe_tat.contracts import (
    DailyTrendPoint,
    DeviceRow,
...
from backend.ebeam.recipe_tat.providers._shape import (
    EquipmentGridRow,
    build_equipment_compare_payload,
    build_equipments_payload,
)
from backend.sem_list.providers import mock as sem_list_mock
...
def _tool_scalars(rng: random.Random, index: int, fab_name: str) -> dict:
    ...
    순번 0(느림)의 speed 대역 U(1.60, 1.75)는 그 자체로 극단적으로
    보이지만, 값을 이렇게까지 밀어야만 하는 이유는 물리적 느림이 아니라
    tat_index 공식의 구조입니다. tat_index = total / expected 이고,
    expected 의 base(r)(레시피 r 의 플릿 평균 meastime)에는 이 장비
    자신의 측정도 그대로 섞입니다 — 칸 하나가 장비 5대뿐인 mock에서는
    이 장비 혼자서 자기 레시피의 플릿 평균을 상당히 끌어올려 자기 지수를
    스스로 완화합니다(감쇠). 정상 폭 U(0.96, 1.04)에서 조금만 벗어난
    값(예: 이전에 쓰던 U(1.12, 1.20))으로는 이 감쇠를 이기지 못해
    프론트엔드 TAT_CEIL(1.10, frontend/app/utils/equipmentSignals.ts)을
    어느 fab에서도 넘지 못했습니다 — R3 뿐 아니라 cd-sem 17개 칸 전부에서
    느림 배지가 구조적으로 뜰 수 없었습니다(2026-08-08 확인). U(1.60,
    1.75)로 올린 뒤에는 관측 시점(2026-08-08, 최근 13일 창 스캔 기준)에
    R3 최댓값이 1.18~1.26, 최악의 칸(M11A)도 1.15~1.24 범위로 TAT_CEIL을
    항상 여유 있게 넘습니다. 표본 수도 25대 실행 안팎(표본 최소치 12
    이상)으로 충분합니다.

    거꾸로 읽으면 유용한 사실 하나가 남습니다: 장비가 5대보다 많은 실제
    사무실 칸에서는 이 감쇠가 훨씬 약해, 같은 정도의 물리적 느림이 mock
    보다 훨씬 높은 지수로 읽힐 것입니다. 즉 TAT_CEIL=1.10 은 mock의 작은
    셀 기준으로 잡은 값이 아니라 오히려 사무실 실 플릿에서는 관대한(=낮게
    잡힌) 쪽일 가능성이 높습니다 — 사무실에서 확인할 사항으로
    recipe_tat/MIGRATION.md 의 "장비별 뷰" OFFICE-VERIFY 목록에도
    남겨뒀습니다. 반대로 이 감쇠 덕분에 다른 네 장비의 지수는 모두
    TAT_FLOOR(0.92) 아래로 내려가(관측 범위 0.78~0.87) 빠름 배지도 매
    fab에서 함께 뜹니다 — speed U(1.60, 1.75)는 "이렇게 느린 장비가
    실재한다"는 주장이 아니라 5대짜리 mock 셀의 감쇠를 상쇄해 두 배지
    상태를 모두 홈에서 시연하기 위한 장치입니다.
    """
    normal_speed = round(rng.uniform(0.96, 1.04), 4)
    normal_workload = round(rng.uniform(0.92, 1.08), 4)

    if index == 0:
        return {"speed": round(rng.uniform(1.60, 1.75), 4),
...
if __name__ == "__main__":
    # Standalone preview:
    #   python -m backend.ebeam.recipe_tat.data
    import pprint
