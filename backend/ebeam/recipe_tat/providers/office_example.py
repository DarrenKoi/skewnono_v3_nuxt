...
``backend/ebeam/_office_meas_hist.py`` (used by every
...
At the office: fill in OPENSEARCH_* in ``backend/.env``,
...
def get_equipments(
    ...
    # 집에서는 테스트로 잡히지 않는 종류이기 때문입니다. 2026-08-09부터
    # EquipmentGridRow 는 NamedTuple 이라 위치가 아니라 이름으로 채웁니다:
    # 자리를 바꿔 넣으려면 이제 그렇게 쓴다고 명시해야 합니다.
    grid: list[EquipmentGridRow] = [
        EquipmentGridRow(
            ...
            meas_counts=int(b["doc_count"]),
            total_meastime=int(b.get("tat", {}).get("value") or 0),
        )
        for b in buckets
    ]
...
if __name__ == "__main__":
    ...
    #     .venv/bin/python -m backend.ebeam.recipe_tat.providers.office
    # (`python path/to/office.py` will NOT work: package imports need -m.)
    # The shared client loads backend/.env itself if the env isn't set.
