...
Home:   .venv/bin/pytest backend/ebeam/recipe_tat
Office: SKEWNONO_RECIPE_TAT_PROVIDER=office .venv/bin/pytest backend/ebeam/recipe_tat
"""
...
def test_office_example_exposes_the_equipment_endpoints():
    ...
    office_example = pytest.importorskip(
        "backend.ebeam.recipe_tat.providers.office_example"
    )
...
def test_mock_rows_carry_real_sem_list_tools():
    ...
    if get_data_provider("recipe_tat") != "mock":
        return
    from backend.ebeam.recipe_tat.providers.mock import _generate_meas_hist
...
def test_request_scope_caps_and_echoes_eqp_ids():
    ...
    from backend.ebeam._analytics_routes import (
        MAX_COMPARE_EQPS,
...
def test_request_scope_eqp_ids_default_to_empty():
    ...
    from backend.ebeam._analytics_routes import resolve_analytics_scope
...
def test_summary_average_excludes_rows_without_a_measurement_time(monkeypatch):
    ...
    from backend.ebeam.recipe_tat.providers import mock
...
def test_sample_eqp_ids_are_the_most_frequent_not_the_alphabetically_first():
    ...
    **후보 집합 자체**가 달라 두 provider 가 서로 다른 장비를 예시로 보여
    ...
    from backend.ebeam.recipe_tat.providers.mock import _top_sample
