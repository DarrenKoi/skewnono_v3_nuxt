...
`backend/.env` (port defaults to 443/SSL).
...
None`. The shared `backend/ebeam/_office_meas_hist.filter_clauses`
...
- **OFFICE-VERIFY — `usage_ratio` and `occupancy` have NO fab normalization at
  ...
  (`frontend/app/utils/equipmentSignals.ts` → `isPeerGroupComparable`,
...
- **OFFICE-VERIFY — `eqp_model_cd.keyword`.** `docs/datatables/hitachi/meas_hist.txt`
...
- **OFFICE-VERIFY — 배지 임계값 (상수 세 개).** 첫 실행에서 아래를 **단일
  ...
  `frontend/app/utils/equipmentSignals.ts`의 `USAGE_FLOOR`,
  ...
  `docs/datatables/hitachi/meas_hist.txt`에 기록합니다 — 이 값은 **측정 점유율**이지
...
Contract gate (`.env` loaded by `backend/conftest.py`):

    SKEWNONO_RECIPE_TAT_PROVIDER=office .venv/bin/pytest backend/ebeam/recipe_tat
