...
At the office: fill OPENSEARCH_* / REDIS_* in ``backend/.env`` and
...
from backend.ebeam._analysis_window import DEFAULT_WINDOW_WEEKS, window_days
from backend.ebeam._office_bm_pm import maintenance_events
from backend.ebeam._office_mdc import MdcChange, changes as mdc_changes
from backend.ebeam._office_meas_hist import (
    EQP_ID_KW,
    FAB_NAME_KW,
    FULL_NAME_KW,
    INDEX,
...
from backend.ebeam._office_msr_cd import (
    RunRef,
    beam_label,
    cd_band,
    has_pickle_clause,
...
from backend.ebeam.tttm.profile import build_parameter_profile
from backend.ebeam.tttm.contracts import (
    DEFAULT_TOLERANCE,
    TOLERANCE_RANGE,
...
def _fleet(tool_slug: str, fab_name: str) -> list[ToolRef]:
    ...
    Same source and same filter as pm_planning's office adapter, because /tttm's
    PM 튜닝 panel joins the two payloads BY eqp_id and a roster derived any other
...
if __name__ == "__main__":  # pragma: no cover
    ...
    #   .venv/bin/python -m backend.ebeam.tttm.providers.office cdsem R3
    ...
    if drops["no_axis"]:
        ...
        print("    Paste this into backend/.env, correcting each axis:")
        print(
            f"    {AXIS_ENV_VAR}="
