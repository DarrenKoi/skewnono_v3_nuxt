...
At the office
─────────────
``backend/.env`` 에 REDIS_* / OPENSEARCH_* / MINIO_* 를 채우고
...
from backend._logging.providers import logger as _LOG
...
from backend.ebeam.device_statistics.oper_order import (
    sort_key as _oper_sort_key,
)
from backend.ebeam.device_statistics.providers.rules import (
...
def _catalog(key: str) -> list[dict[str, Any]]:
    ...
    if raw is None:
        raise LookupError(
            f"Redis key {key!r} not found — check REDIS_* in backend/.env "
...
if __name__ == "__main__":
    ...
    #   .venv/bin/python -m backend.ebeam.device_statistics.providers.office
    import sys
