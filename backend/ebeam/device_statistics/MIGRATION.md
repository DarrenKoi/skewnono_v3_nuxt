...
- **일곱 개**를 구현합니다: 기존 다섯 개에 더해 `write_weekly_snapshot`,
  ...
  않습니다.
- **External importer — do not break this.** `backend/ebeam/_analytics.py`'s `lot_metadata()` lazily imports `get_device_desc` and
...
- **Do NOT touch `providers/mock.py`'s `_lot_index` export or the recipe_tat
  import that reads it.** `backend/ebeam/recipe_tat/providers/mock.py` imports `_lot_index` directly from
...
- Notes: **huge-payload endpoint.** Called with no `lot_cds` filter, this
  ...
  (`frontend/app/composables/useRecipeStatisticsApi.ts`) always joins
...
- Notes: **huge-payload endpoint**, same shape of concern as
  ...
  (`frontend/app/composables/useDeviceStatisticsApi.ts`) always
...
잽니다 (`frontend/app/utils/ruleEngine.ts` 의 `groupCaps`/`effectiveCap`).
...
## Verify
    ...
    SKEWNONO_DEVICE_STATISTICS_PROVIDER=office .venv/bin/pytest backend/ebeam/device_statistics
