...
- Requires `REDIS_HOST` / `REDIS_PORT` / `REDIS_PASSWORD` in
  `backend/.env`, resolved through `_runtime/office_redis.py`.
...
`backend/__init__.py` maps to a JSON 503, via `unreachable()` in
...
are called by `backend/_auth/middleware.py:41`
...
50-entry cap and every branch of the outage table without a server:
    ...
    .venv/bin/pytest backend/access_control
...
date:
    ...
    SKEWNONO_ACCESS_CONTROL_PROVIDER=office .venv/bin/pytest backend/access_control
