...
``frontend/.output/public``, which does not exist in a fresh checkout.
...
from backend._runtime import env
from backend._spa import serving
...
def build(tmp_path, monkeypatch):
    ...
    """
    root = tmp_path / "frontend" / ".output" / "public"
...
def test_traversal_cannot_read_a_file_outside_the_build(client, build, path):
    ...
    which also holds backend/ and the .env carrying the Redis and
...
def test_create_app_claims_no_static_route_of_its_own():
    ...
    `Flask(__name__)` registers /static/<filename> against backend/static/
...
def test_spa_dir_matches_the_path_the_deploy_tooling_hardcodes():
    ...
    assert (root / "backend" / "_runtime" / "env.py").is_file()
    assert (root / "frontend").is_dir()
    assert env.spa_dir() == root / "frontend" / ".output" / "public"
...
def test_the_logging_middleware_reads_the_same_flag_name():
    ...
    source = (
        env.project_root() / "backend" / "_logging" / "activity.py"
