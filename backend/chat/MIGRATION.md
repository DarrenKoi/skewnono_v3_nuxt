...
`root` 는 `SKEWNONO_CHAT_RAG_ROOT`, 미설정이면 `backend/chat/_rag` 입니다
...
```text
  chat/answer  office  RAG checkout at /project/workSpace/backend/chat/_rag
...
**배치.** Co-location 루트는 `backend/chat/_rag/` 입니다(RAG 측 확인
...
`backend/.env` 에도 넣지 않습니다. 앞머리 밑줄이 이 배치를 안전하게
만드는 전부입니다.
...
| 배포 산출물 공유 | 의도된 배치입니다 — `pack.py` 가 `backend` 을 통째로 싣습니다 |
...
| Deploy pack | `_` 경로를 걷지 않는 규칙은 없지만 `backend` 통째로 복사하므로 **함께 실립니다**(의도). 대신 `.git` 은 `PRUNE_DIRS` 로 버립니다. |
| ruff | `.gitignore` 를 따르므로 무시합니다. |
| pytest | `.gitignore` 를 따르지 **않으므로** `pyproject.toml` 의 `--ignore=backend/chat/_rag` 가 막습니다. |
| git | `.gitignore` 의 `backend/chat/_rag/`. `skewnono_rag/` 갱신은 RAG 측 전달로 통째 교체합니다(사내 저장소라면 `git -C backend/chat/_rag/skewnono_rag pull`). Submodule 은 쓰지 않습니다 — 사무실은 GitHub 에 로그인할 수 없고 RAG 저장소는 사내 전용입니다. |
