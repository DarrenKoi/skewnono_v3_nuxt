...
out at ``backend/chat/_rag/`` by default: the leading underscore is what
...
repo (its own ``.git`` keeps working: ``git -C backend/chat/_rag pull``).
...
from backend.chat.contracts import KnowledgeUnavailable
...
def import_rag(module: str) -> ModuleType:
    ...
    if root is None:
        raise KnowledgeUnavailable(
            ...
            "or clone it to backend/chat/_rag."
        )
