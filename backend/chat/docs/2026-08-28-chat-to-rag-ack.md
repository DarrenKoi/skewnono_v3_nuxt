...
- **deploy 가 thread 를 덮어쓸 뻔했습니다.** `scripts/deploy/pack.py` 는
  ...
  PC 의 `backend/chat/chat.db` 도 같이 실려 cloud 의 `chat.db` 위에
  overlay 될 상황이었습니다. `*.db` 를 prune 목록에 넣었습니다.
- **cloud 에서는 `SKEWNONO_CHAT_DB` 를 overlay 바깥 절대 경로로.** 기본 경로
  `backend/chat/chat.db` 는 overlay 되는 트리 안입니다. prune 으로 덮어쓰기는
...
- 집: `.venv/bin/python -m pytest backend/chat tests/test_pack_deploy.py -q`
