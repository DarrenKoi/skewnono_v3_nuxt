...
```bash
cp backend/chat/knowledge/providers/office_example.py backend/chat/knowledge/providers/office.py
cp backend/chat/scope/providers/office_example.py     backend/chat/scope/providers/office.py
git clone <rag-repo> backend/chat/_rag      # 또는 기존 checkout 을 이 경로로 이동
```

두 template 모두 **완성된 구현**이므로 복사본을 고치지 않습니다. `.env` 는
`backend/.env.example` 의 "Office RAG" 블록을 따릅니다
...
| 4. env vars | 완료 | `backend/.env.example` |
...
RAG 는 별도 git 저장소로 남고, `backend/chat/_rag/` 에 checkout 합니다.
...
| pytest 가 `backend` 전체를 수집 | `pyproject.toml` 의 `--ignore=backend/chat/_rag`. |
| ruff | `.gitignore` 를 따르므로 자동 제외. |
| deploy pack | `backend` 통째로 복사되어 **함께 배포됩니다**. `.git` 만 prune 합니다. |
...
- 집: `.venv/bin/python -m pytest backend/chat -q` — seam 은 가짜
...
| --- | --- |
| `import src.retrieve.serve` / `import src.retrieve.agent` (루트 = `SKEWNONO_CHAT_RAG_ROOT`, 기본 `backend/chat/_rag`) | 루트 바로 아래에 `src/` 패키지가 있어야 합니다. `chat/rag.py` 는 `{root}/src` 디렉터리가 없으면 "checkout 없음" 으로 봅니다. 이름을 바꾸면(제안 1) 알려 주세요. |
...
- [ ] `backend/chat/_rag/src/retrieve/serve.py` 와 `agent.py` 가 위 세 함수를
...
- [ ] `.venv/bin/python -m pytest backend/chat -q` 가 사무실에서 통과합니다
