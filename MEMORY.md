...
- Phase 1 mock backend is a local Flask server under `backend/`. Nitro's dev proxy forwards `/api/*` to `NUXT_API_TARGET` (default `http://localhost:5000`); the frontend reads the base path from `runtimeConfig.public.apiBase` (default `/api`). Same wiring carries into Phase 2/3 — only `NUXT_API_TARGET` changes.
