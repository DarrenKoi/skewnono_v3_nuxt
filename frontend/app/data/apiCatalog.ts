...
// No DELETE: the one deletable resource is an API token, and revoking one is
// browser-only by policy (`api_tokens/routes.py` rejects token-authenticated
// callers). Users come to Settings → API Tokens for it.
...
export const authNotes = [
  ...
  'API는 사용자당 5초에 50회로 rate limit이 걸려 있습니다. 배치 스크립트에서 반복 호출할 때는 호출 사이에 간격을 두십시오.',
  '토큰 인증 요청은 사용자 활동 점수에는 반영되지 않지만, 운영 로그에는 api_token_id와 함께 남습니다.',
  '토큰 발급과 폐기는 사람 세션 전용이라 API로는 할 수 없습니다. 새 토큰을 만들거나 쓰던 토큰을 없앨 때는 Settings의 API Tokens 화면으로 직접 오십시오.'
]
