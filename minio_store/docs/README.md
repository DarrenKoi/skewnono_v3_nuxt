...
이 폴더는 `minio_store` 패키지의 사용법과 MinIO 자체에 대한 학습 자료를
함께 정리합니다.
...
- `usage.md`: `minio_store` wrapper 자체의 설정 방법, CRUD 예제, presigned
...
`minio_store`는 `minio` Python SDK 위에 얇게 올린 wrapper입니다. bucket /
