"""Create fab_inform_notes rollover index, template, alias, and ISM policy."""
...
INDEX_ALIAS = "fab_inform_notes"
POLICY_ID = "fab_inform_notes_retention_policy"
TEMPLATE_NAME = "fab_inform_notes_template"
...
DATE_FIELDS = ("down_dt", "equp_dt", "up_dt", "hub_load_tm")
NOTE_FIELDS = ("note_comment", "zzproblem", "hltext")
...
def create_skewnono_client() -> Any:
    ...
    if not OPENSEARCH_PASSWORD:
        raise RuntimeError(
            ...
            "ops_index_mgmt/fab_inform_notes.py before running this script."
        )
...
def setup_fab_inform_notes(client: Any | None = None) -> dict[str, Any]:
...
# ---------------------------------------------------------------------------
# Reference: ingesting a dataframe into fab_inform_notes
# ---------------------------------------------------------------------------
...
# before = len(notes_df)
# notes_df["doc_id"] = notes_df["doc_id"].astype("string").str.strip()
# notes_df = notes_df[notes_df["doc_id"].notna() & (notes_df["doc_id"] != "")]
# notes_df = notes_df.drop_duplicates(subset="doc_id", keep="first")
# print(f"filtered {before - len(notes_df)} rows (empty or duplicate doc_id)")
#
