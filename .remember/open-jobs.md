...
- **Self-id review: every finding closed** (`6066109`, `d5730e9`, `27e7099`, `5683545`, `5bde1b9`): `/identify` requires `emp_nm` server-side; preflight checks `SKEWNONO_SECRET_KEY`; the header identity pill ships the 미검증 badge and the 본인이 아닙니다 release (verified in-browser: declare → badge → release → fallback); `/api/me` falls back to the stored declared name when the directory row is nameless (home mock always fabricates a name — only the planted-None tests cover the cloud path); and the hardening pass: `SESSION_COOKIE_SAMESITE=Lax`, 64-char bound on empno/emp_nm (mirrored client-side), reserved ids (`anonymous`/`local-dev`) undeclarable, `SKEWNONO_TRUST_PROXY` documented in `.env.example`. Only the office-side `flask_modules` sync (above) remains.
