...
- **Self-id review gaps 1–3** (`6066109`, `d5730e9`, `27e7099`): `/identify` requires `emp_nm` server-side; preflight checks `SKEWNONO_SECRET_KEY`; the header identity pill ships the 미검증 badge and the 본인이 아닙니다 release (verified in-browser: declare → badge → release → fallback). Plus `5683545`: `/api/me` now falls back to the stored declared name when the directory row is nameless — the home mock always fabricates a name, so only the planted-None tests cover the cloud path.
