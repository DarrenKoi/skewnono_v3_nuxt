...
- [ ] Office verify recipe_tat + health (STATUS.md says 구현완료, never run on office data) — **OFFICE-ONLY, proven 2026-07-24**: attempted from home and it cannot work. Company Redis `10.156.133.126:10121` → TimeoutError; `.env` has no `OPENSEARCH_*` keys, so `recipe_tat` office raises `OPENSEARCH_HOST is not set`. The `cp office_example.py office.py` step is ALREADY DONE for all 6 adapters on the Mac mini. At the office: site auto-detects (`PC*` → office; sanity-check `python -c "from back_dev_home._runtime.site import detect_site; print(detect_site())"` — prints `home` here), run office-mode contract tests, eyeball recipe 현황 (ranking is uncapped: `DEFAULT_LIMIT = 0` — watch first-load latency), bump STATUS.md → office + date · since 2026-07-21
