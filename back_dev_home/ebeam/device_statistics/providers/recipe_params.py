...
#   Align 16 — 정상 파라미터 중앙값(약 6)의 문턱(중앙값×2 ≈ 12)을 넘습니다.
#              제외가 빠지면 Sample recipe 마다 outlier 가 하나씩 생깁니다.
#
# ★ Align 은 **16 을 넘지 않습니다** (user-confirmed 2026-08-10). 40 이었던 것은
#   지어낸 값이고, 이제 틀린 것으로 확인됐습니다.
#
#   이 정정이 왜 지금 필요한지가 기록해 둘 만합니다. 정확 일치 시절에는 40 이
#   16/13/9/5 어디에도 맞지 않아 **아무 버킷에도 안 들어가** 보이지 않았는데,
#   2026-08-10 에 버킷이 구간이 되면서 para_over_16 으로 들어갔습니다. Sample
#   recipe 가 전체의 25% 라 그것만으로 "recipe 의 30% 가 16 초과 파라미터를 갖는"
#   그림이 됐습니다 — 실물은 3% 입니다. 지어낸 값이 조용히 자고 있다가 정의가
#   바뀌는 순간 깨어난 경우이고, OFFICE-VERIFY 표시가 있어서 어느 쪽이 사실인지
#   물어볼 수 있었습니다.
#
#   확인된 상한(16)을 그대로 쓰는 것은 outlier 제외 회귀를 살려 두기 위해서입니다.
#   중앙값이 8 을 넘게 올라가면 문턱이 16 을 넘어 이 회귀가 조용히 죽으므로,
#   그때는 이 값이 아니라 회귀의 설계를 다시 봐야 합니다.
#
# OFFICE-VERIFY: 정확한 point 값은 여전히 확인된 바 없습니다 — 아는 것은
# "16 이하" 라는 것뿐이고, "규칙이 걸리는 쪽" 이라는 성질만 재현합니다.
# 이름의 표기는 확인된 사실입니다.
#
