...
from back_dev_home.ebeam.cdsem.storage.data import get_storage, get_storage_unavailable

bp = Blueprint("cdsem_storage", __name__)
...
@bp.get("/cdsem/storage")
def storage():
    rows = get_storage(_parse_fac_ids())
    return jsonify(rows)


@bp.get("/cdsem/storage-unavailable")
