...
def _fab_base(fab_name: str) -> str:
    """Strip sub-fab suffix so M15A → M15. Personality maps key off the
    base since extending FAB_CLASS_MIX / FAB_MEASTIME_MULTIPLIER for every
    A/B/C variant would triple their size for negligible UX gain."""
...
def _pick_recipe_for_fab(
    ...
    class_name = rng.choice(FAB_CLASS_MIX.get(_fab_base(fab_name), DEFAULT_CLASS_MIX))
...
def _generate_meas_hist() -> tuple[MeasHistRow, ...]:
    ...
    for index in range(TOTAL_MEAS_ROWS):
        ...
        fab_multiplier = FAB_MEASTIME_MULTIPLIER.get(_fab_base(fab_name), 1.0)
...
def _filter_rows(
    ...
    fab_norm = fab_id.upper() if fab_id else None

    out: list[MeasHistRow] = []
    for row in _generate_meas_hist():
        if tool_type and row["tool_type"] != tool_type:
            continue
        # Strict fab_name match. R3's fac_id is shared by R3 and R4 fab_names,
        # so falling back to fac_id matching would leak R4 rows into an R3
        # query (and vice versa).
        if fab_norm and row["fab_name"].upper() != fab_norm:
            continue
        if lot_cd and row["lot_cd"] != lot_cd:
            continue
        # ISO-8601 strings sort lexicographically by date, so prefix-compare
        # against YYYY-MM-DD bounds without parsing 6000× per scan.
