...
def _resolve_dates() -> tuple[str, str]:
    ...
    Same reasoning as recipe_tat's _resolve_dates — ANCHOR_TIME is fixed
    at process import, so defaulting to wall-clock now() would drift past
    the data ceiling for long-running Flask processes.
    """
...
def fail_issue_align_ranking(tool_slug: str):
    ...
    rows = get_align_ranking(tool_type, fab_id, start_date, end_date, limit=limit, lot_cd=lot_cd)
    return jsonify({
        "tool_type": tool_type,
        "fab_id": fab_id,
        "start_date": start_date,
...
def fail_issue_meas_ranking(tool_slug: str):
    ...
    rows = get_meas_ranking(tool_type, fab_id, start_date, end_date, limit=limit, lot_cd=lot_cd)
    return jsonify({
        "tool_type": tool_type,
        "fab_id": fab_id,
        "start_date": start_date,
