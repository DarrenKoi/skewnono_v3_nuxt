...
def _enrich(row: MeasHistRow) -> FailRow:
    rng = _row_rng(row["id"])
    fab_base = _fab_base(row["fab_name"])

    align_fail_rate = FAB_ALIGN_FAIL_RATE.get(fab_base, DEFAULT_ALIGN_FAIL_RATE)
    meas_fail_rate = FAB_MEAS_FAIL_RATE.get(fab_base, DEFAULT_MEAS_FAIL_RATE)
