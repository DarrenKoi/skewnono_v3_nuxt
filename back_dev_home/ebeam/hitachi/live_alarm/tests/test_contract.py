...
quiet fab is legitimately nothing. Assumptions about the fab whitelist are
...
# the office writer's Redis registry, so both must call it "not_configured".
...
def test_every_event_carries_a_known_kind():
    # ALID_KIND is SKEWNONO's statement of the Redis key layout the writer job
    # shares (contracts.py header). If the office writer ever pushes a third
    # alid, the board would render it with no kind — so this stays UNFENCED,
    ...
    for event in data.get_board(TOOL_TYPE, CONFIGURED_FAB)["events"]:
        ...
        assert expected is not None, (
            ...
            f"in ALID_KIND ({sorted(ALID_KIND)}) — writer and reader have drifted"
        )
...
def test_unknown_fab_is_not_configured_with_an_empty_board():
    ...
    # providers: a fab absent from the mock whitelist / the writer's Redis
    # registry is "미설정", never a healthy quiet board. A typo'd fab must
    # look the same at home as at the office.
    board = data.get_board(TOOL_TYPE, UNKNOWN_FAB)
    assert_matches(board, LiveAlarmPayload)
    assert board["feed_status"] == "not_configured"
    assert board["events"] == []
    assert board["polled_at"] is None
...
def test_configured_fab_reports_a_heartbeat_state():
    if not _is_mock():
        ...
        # only once the writer job has registered it, and "not_configured" is
        # then the correct answer rather than a failure. Skipped rather than
    ...
    assert board["polled_at"] is not None
