"""The import bridge to the co-located office RAG checkout (``src.*``)."""
...
def test_root_without_a_src_package_is_not_a_checkout(tmp_path, monkeypatch):
...
def test_import_puts_the_root_on_sys_path_once(tmp_path, monkeypatch):
    (tmp_path / "src").mkdir()
    monkeypatch.setenv("SKEWNONO_CHAT_RAG_ROOT", str(tmp_path))
    fake = types.ModuleType("src.retrieve.serve")
    monkeypatch.setitem(sys.modules, "src.retrieve.serve", fake)
