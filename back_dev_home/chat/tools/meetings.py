...
def build_search_meeting_summaries_tool(
    access_scope: AccessScope,
    evidence_budget: EvidenceBudget | None = None,
):
    """Build a meeting search tool closed over the caller's server-owned scope."""
    budget = evidence_budget or EvidenceBudget.from_config()

    @tool("search_meeting_summaries", response_format="content_and_artifact")
    def search_meeting_summaries(query: str) -> tuple[str, dict]:
        """Search approved meeting summaries for evidence relevant to the query."""
        started = time.perf_counter()
        rows = knowledge_data.search_meeting_summaries(query, {}, access_scope, 5)
        trace = {
            "tool_name": "search_meeting_summaries",
            "query": query,
            "result_count": len(rows),
            "duration_ms": int((time.perf_counter() - started) * 1000),
            "status": "success" if rows else "empty",
        }
        content, bounded = budget.prepare(rows, "No meeting evidence found.")
        trace["result_count"] = len(bounded)
        return content, {"sources": bounded, "trace": trace}

    return search_meeting_summaries
