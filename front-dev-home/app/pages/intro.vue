...
const pageGuides: PageGuide[] = [
  ...
  },
  {
    ...
    section: 'lab',
    purpose: '계측 지식을 자연어로 묻고 답변의 근거까지 함께 확인합니다.',
    description: '왼쪽에 대화 목록이, 가운데에 주고받은 메시지가 놓입니다. 첫 질문을 보내면 그 문장이 대화 제목이 되어 목록에서 찾기 쉬워집니다. 답변에는 응답 시간과 토큰 수가 붙습니다. 인용한 출처는 칩으로 달리고 누르면 인용된 원문과 그림을 그 자리에서 봅니다. 이어서 물어볼 질문이 함께 제안되므로 누르면 입력창에 채워집니다. 답변마다 좋아요와 싫어요로 평가를 남길 수 있습니다. 답변을 기다리다 새로고침하거나 다른 화면에 다녀와도 진행 중이던 답변을 이어서 받습니다.',
    users: '전체 사용자',
    notes: [
      '모델과 시스템 프롬프트는 백엔드가 정하므로 화면에서 고르지 않습니다.',
      '이 화면은 실험실의 다른 화면이 하는 조회·계산 대신 대화를 다루므로 메뉴에서도 구분선 아래에 둡니다.'
    ]
  },
  {
    id: 'afm-tools',
