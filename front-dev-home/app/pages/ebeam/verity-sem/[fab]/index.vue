...
const tools = computed(() => {
  return Array.from({ length: 4 }, (_, i) => ({
    id: `${fabId.value}-VERITY-${String(i + 1).padStart(3, '0')}`,
    status: 'online',
    currentJob: Math.random() > 0.7 ? `JOB-${Math.floor(Math.random() * 1000)}` : null,
...
<template>
  <div>
    <div class="mb-6">
      <h1 class="text-2xl font-bold">VeritySEM - {{ fabConfig.label }}</h1>
