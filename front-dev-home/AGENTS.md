...
This repository is a Bun-based Nuxt 4 frontend. Keep application code under `app/`:
...
- `bun run dev` starts Nuxt on `http://localhost:3100`
- `bun run dev:remote` binds to `0.0.0.0` for remote access
- `bun run build` creates the production build
- `bun run preview` serves the built app locally
- `bun run lint` runs ESLint
- `bun run typecheck` runs Nuxt/Vue TypeScript checks

CI currently enforces `bun install --frozen-lockfile`, `bun run lint`, and `bun run typecheck`.
...
## Testing Guidelines
There is no dedicated unit or E2E test suite yet. Until one is added, treat `bun run lint` and `bun run typecheck` as required before every PR. For new data-access code, keep the current mock-data pattern: composables should return functions that resolve `Promise<T>` so backend integration can replace mocks later with minimal churn.
