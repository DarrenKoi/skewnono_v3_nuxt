...
3. **ABBA 불확실성 표시**: self-ABBA ±0.05 nm를 tolerance 슬라이더의
   ...
   **tolerance 상한 논의는 종결되었습니다 — 상한 `0.20` 을 유지합니다
   (user-confirmed 2026-08-16).** 이 문서의 앞선 판과 선행 검토 4장은 Kawada
   2009 전문의 matching target **±0.25 nm** 를 근거로 상한을 0.30 nm 로 올리고
   기본값을 0.25 로 두자고 했으나, **사용자가 0.20 이 타당하다고 판정했습니다.**
   2009년 논문의 목표치는 그 시점 노드의 값이며 현재 팹의 매칭 스펙을 구속하지
   않습니다. 실제 스펙을 아는 쪽의 판정이 문헌값보다 우선합니다.

   **다시 제기하지 마십시오.** 이 항목은 두 번 제기되어 두 번 다 같은 답을
   받았습니다.

   남는 것은 문헌값이 아니라 **목 데이터** 쪽 사실 하나입니다. fixture 의 최대
   셀 스큐가 정확히 `0.200` 이라 노브를 끝까지 올리면 모든 쌍이 통과하므로,
   데모에서는 "상한에서도 불합격인 장비 그룹"을 한 번도 볼 수 없습니다. 이는
   계약 문제가 아니라 목 값 문제이므로, 고친다면 `mock.py` 에서 한 쌍을 상한
   위로 올리는 편이 맞습니다. 우선순위는 낮습니다.

   이 항목의 나머지(self-ABBA ±0.05 nm 를 눈금 위 **바닥선**으로 표시, 셀별
   표본수와 n &lt; 50 경고)는 그대로 유효합니다.
...
2. ~~A-3 의 tolerance 상한 정정~~ **불필요** — 상한 `0.20` 유지로 종결
   (user-confirmed 2026-08-16). A-3 의 나머지(ABBA 바닥선, 표본수 경고)만
   남습니다.
