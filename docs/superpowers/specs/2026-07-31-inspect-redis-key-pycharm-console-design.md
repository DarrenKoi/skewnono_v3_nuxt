...
- 명령행 인자 처리와 `main()` 함수를 제거합니다.
- `scripts/inspect_device_info_keys.py`가 이 모듈의 도우미를 가져올 때 Redis 조회가
  실행되지 않도록 `if __name__ == "__main__"` 실행 가드는 유지합니다. 이 가드
  안에서 대입한 이름도 모듈 전역 변수이므로 PyCharm 콘솔에서 확인할 수 있습니다.
...
이 파일은 명령행 인자 기반 도구가 아니라 대화형 조사 스크립트로 전환됩니다.
다른 스크립트가 가져오는 `_human_bytes`와 `describe_dataframe` 함수는 유지하고,
모듈을 import할 때 Redis에 연결하지 않는 기존 동작도 보존합니다. 검증에서는
문법 컴파일, import 사용처, 정적 테스트, Markdown lint를 확인합니다.
