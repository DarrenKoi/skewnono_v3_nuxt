...
requiredFovNm(patternCount, pitchNm, marginRatio) → number | null
...
| 여유 마진 | ✓ | 10% | 각 변 기준. 프리셋 0 / 5 / 10 / 15 / 20% |
...
패턴은 화면 가운데에 놓이고 **상하좌우 각 변에 빈 공간이 필요합니다**
(통상 10%, 사용자 확인 2026-07-25). 마진은 렌더링 장식이 아니라 **계산에
들어가는 값**입니다.

```text
필요 FOV = (패턴 수 × pitch) ÷ (1 − 2 × 마진비율)
```

각 변 10%면 좌우 합쳐 20%가 비므로 패턴은 FOV의 80%를 차지합니다.
예: 8패턴 × 90 nm = 720 nm → `720 ÷ 0.8` = **필요 FOV 900 nm** → 150 K.

마진을 빼고 계산하면 180 K(FOV 750 nm)가 추천되는데, 이는 **패턴이 화면 끝에
붙는 배율**입니다. 마진은 두 제약 모두를 불리하게 만듭니다 — 배율이 내려가고,
FOV가 넓어져 nm/px가 커지고, 따라서 px/CD도 줄어듭니다. 어느 한쪽에만
반영하면 안 됩니다.

프리셋(0/5/10/15/20%)으로 받습니다. 자유 입력이 아닌 이유는 배율이 이산값이라
마진이 연속적으로 반응하지 않기 때문입니다 — 아래 민감도 표에서 5%와 10%가
같은 150 K로 떨어지는 것처럼, 프리셋 간격이면 계단 변화를 관찰하기에 충분합니다.

**민감도 패널을 상시 노출합니다.** 마진 프리셋별 필요 FOV와 추천 배율을 함께
보여주어, 지금 마진에 여유가 얼마나 남아 있는지(한 칸 더 올려도 배율이 그대로인지,
아니면 바로 떨어지는지) 판단할 수 있게 합니다.
...
검증: 마진비율은 `0 <= r < 0.5`. `r >= 0.5`면 패턴이 들어갈 공간이 없으므로
`requiredFovNm`은 `null`을 반환합니다. 프리셋만 쓰므로 UI에서는 도달할 수 없으나
함수 계약으로 막아둡니다.
...
- **① 전체 FOV** — 가로 스트립에 bar/space를 패턴 수만큼 렌더. FOV 안에 다
  들어오는지(**FOV 제약**)를 보여줍니다. 양끝의 **마진 영역을 빗금으로 표시**하고
  `마진 / 패턴 span / 마진` 치수를 답니다.
...
시뮬레이션은 **상하좌우 네 변 모두**에 마진을 렌더합니다. 패턴은 중앙
`(1 − 2r) × (1 − 2r)` 영역에 놓입니다. 모식도 ①이 가로 방향만 보여주는 데
비해, 시뮬레이션은 "패턴이 가운데, 사방에 빈 공간"이라는 실제 촬영 구도를
그대로 보여주는 역할을 맡습니다.
...
```text
requiredFovNm = (패턴 수 × pitch) / (1 - 2 × 마진비율)
...
- `requiredFovNm`: 8패턴 × 90 nm, 마진 0 → 720 / 마진 0.1 → 900 /
  마진 0.2 → 1200. `r >= 0.5` → null
- 마진이 추천을 바꾸는 것: 같은 입력에 마진 0 → 180 K, 마진 0.1 → 150 K.
  **이 회귀 테스트가 마진을 계산이 아닌 렌더링에만 반영하는 실수를 잡습니다**
- 추천: 성립 Mag 없음 / 512 통과 / 512 미달·1024 통과 / 둘 다 미달 4개 분기

## 명시적 가정

**마진은 가로·세로에 같은 비율로 적용합니다.** 픽셀 설정이 정사각(`512,512`)이고
FOV도 정사각이므로 단일 비율 하나로 양방향을 덮습니다. line/space 패턴에서
실제로 배율을 구속하는 것은 가로 방향(pitch 방향)이므로 `requiredFovNm` 계산은
가로 기준이며, 세로 마진은 시뮬레이션 렌더링에만 반영됩니다. 비정사각 픽셀
설정이 실제로 들어오면 이 가정을 재검토해야 합니다.

**패턴 수는 pitch 주기 수로 셉니다.** bar 개수가 아니라 `bar + space` 주기
개수입니다. 8을 넣으면 패턴 span은 `8 × pitch`입니다.
