...
이 프로젝트의 도메인은 반도체 계측입니다. UI 뒤에는 **로버스트 통계**와 **물리 좌표 변환**이 깔려 있고, 둘 다 `utils/`의 **순수 함수**로 구현돼 있습니다(프레임워크·DOM·Nuxt 의존성 0 → `node --test`로 검증. 테스트 규율은 `13-testing/` 참고).
...
- **`× 1.4826`** = `1 / Φ⁻¹(0.75)`. 정규분포를 가정하면 MAD를 **표준편차 σ의 일치추정량(consistent estimator)**으로 만들어 주는 보정 상수. 이걸 곱하면 MAD가 "로버스트한 σ"가 됩니다.
