...
예전 학습 노트(`07-code-patterns/`)는 "단위 테스트 기반 마련"을 *다음에 해볼 일*로 적어 두었습니다. 지금은 **현실이 되어** 프론트엔드에 57개의 `*.test.ts` 파일이 있습니다. 이 문서는 그 테스트 규율을 다룹니다.
...
- 테스트 대상이 **순수 함수**뿐이라 브라우저 DOM·jsdom·Vue 렌더링이 필요 없습니다. Vitest의 강점(컴포넌트 마운트, jsdom)이 필요 없으니 도입 비용만 남습니다.
...
- **테스트 러너도 의존성이다.** 순수 함수만 테스트하면 Vitest의 무게 없이 Node 내장 러너로 충분하다.
- **테스트 가능성 = 좋은 경계.** "이걸 어떻게 테스트하지?"가 안 풀리면, 보통 계산과 부수효과가 안 갈라진 것이다.
- **엣지 케이스는 테스트에 박제한다.** `>` vs `>=`, 빈 배열, `NaN`/`null` — 코드는 잊지만 테스트는 기억한다.
