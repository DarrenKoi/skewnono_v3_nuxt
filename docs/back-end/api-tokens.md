...
순서가 중요합니다. 토큰 경로(1)가 쿠키 경로(2)보다 **먼저** 시도되므로, 같은 사용자가 브라우저에서 `LASTUSER` 쿠키로 로그인되어 있어도 `Authorization` 헤더가 붙은 호출은 토큰 경로로 식별됩니다.

토큰이 형식상 존재하지만 폐기되었거나 위조된 경우(1번 경로에서 매치 실패) 미들웨어는 **즉시 401을 반환하며 리다이렉트하지 않습니다**. 머신 호출자가 리다이렉트 루프에 빠지지 않도록 하기 위함입니다.
