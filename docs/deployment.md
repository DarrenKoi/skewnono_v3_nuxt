...
`harakiri = 120` 은 스케줄러 작업을 죽이지 않습니다 — harakiri 타이머는 **요청**
단위로 걸립니다. 120 인 이유: 요청 안에서 도는 tool-FTP 페치의 자체 한도
(`ftp_handler` `host_timeout` 60초)보다 길어야, 멈춘 장비를 SIGKILL(워커 전체와
동승한 요청까지 죽음)이 아니라 어댑터의 정상 503 으로 답하고 MinIO 캐시 기록도
보존됩니다. `threads = 4` 도 같은 진단의 산물입니다 — FTP 페치가 요청 슬롯을
