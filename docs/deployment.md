...
`harakiri = 380` 은 스케줄러 작업을 죽이지 않습니다 — harakiri 타이머는 **요청**
단위로 걸립니다. 이 값이 커야 하는 이유는 하나입니다: 앱이 스스로 거는 모든
한도보다 위에 있어야, 멈춘 요청을 SIGKILL(워커 전체와 동승한 요청까지 죽음)이
아니라 정상 코드로 답할 수 있습니다. 한도는 둘입니다 — tool-FTP 페치의
`ftp_handler` `host_timeout` 60초(503 + MinIO 캐시 보존), 그리고 chat 한 턴의
`SKEWNONO_CHAT_ANSWER_TIMEOUT` 상한 360초에 어댑터 여유 5초를 더한 365초(504).
380 은 그 365 위의 여유입니다. **chat 상한을 올리면 이 값도 같이 올립니다**
— 안 그러면 가장 느린 턴이 504 대신 502 가 됩니다(2026-09-01, 120 -> 380). `threads = 4` 도 같은 진단의 산물입니다 — FTP 페치가 요청 슬롯을
