...
`harakiri` 는 스케줄러 작업을 죽이지 않습니다 — harakiri 타이머는 **요청**
단위로 걸립니다. 이 값이 커야 하는 이유는 하나입니다: 앱이 스스로 거는 모든
한도보다 위에 있어야, 멈춘 요청을 SIGKILL(워커 전체와 동승한 요청까지 죽음)이
아니라 정상 코드로 답할 수 있습니다.

**클라우드 호스트의 실제 값은 720 입니다**(user-confirmed 2026-09-01). 저장소의
`wsgi.ini` 는 배포되지 않으므로 그 파일은 "클라우드 사본이 담아야 하는 값" 의
기록일 뿐이며, 실제 값과 어긋나 있으면 다음 사람이 잘못된 계산을 합니다.

그 아래에 있어야 하는 앱 자체 한도는 이제 **하나**입니다 — tool-FTP 페치의
`ftp_handler` `host_timeout` 60초(503 + MinIO 캐시 보존).

**Chat 은 2026-09-01 부터 이 계산에서 빠졌습니다.** 한 턴이 백그라운드
워커에서 답해지고 `POST` 는 밀리초 만에 202 로 돌아오므로, RAG 예산
(기본 240초, 상한 300초)은 더 이상 harakiri 아래에 있지 않습니다. 그전에는
상한 360초 + 어댑터 여유 5초 = 365초가 이 값의 바닥이었고, 그래서 380 이었습니다
(2026-09-01, 120 -> 380 -> chat 이 빠지며 무관해짐). **이제 chat 예산을 올려도
이 값을 따라 올릴 필요가 없습니다.**

`threads = 4` 도 같은 진단의 산물입니다 — FTP 페치가 요청 슬롯을
점유하는 동안 대기열의 요청들이 인그레스 타임아웃(502)으로 죽지 않도록
슬롯을 8→16 으로 늘렸습니다. `max-requests = 1000` 은 워커 1 을 주기적으로 재생성하며,
그때마다 스케줄러가 앱 부팅 시간만큼 끊겼다 복구됩니다.

돌고 있는지 확인하려면 `GET /api/health/jobs` (관리자)를 봅니다. 작업 하나가
하루에 여러 줄로 보이면 선출이 실패한 것입니다.

각 기록의 `ts` 는 스케줄러 타임존과 같은 KST(`+09:00`)입니다. 이 값을 바꾼
배포 이전에 쌓인 오피스 Redis 리스트의 옛 기록은 `+00:00` 으로 남아 있으므로,
한 화면에 두 오프셋이 섞여 보일 수 있습니다. 둘 다 오프셋이 명시된 값이라
그대로 읽으면 되고, 리스트가 한 바퀴 돌면 자연히 사라집니다.

스케줄러를 멈춰야 하면 `SKEWNONO_SCHEDULER_ENABLED=0` 을 설정하고 재기동합니다.
코드 변경은 필요하지 않습니다.

### 매일 밤 재기동과 로그 정리 (`touch-reload`)

호스트 유지보수용 작업 두 개가 자정 직후에 돕니다. 조용한 시간대(01:00–08:00)
밖에 두는 것이 의도입니다 — 하루 업무를 **몇 분 전에 기동한 프로세스**로
시작하는 것이 목적이기 때문입니다.

| 시각 (KST) | 작업 | 하는 일 |
| --- | --- | --- |
| 00:05 | `uwsgi_touch_reload` | `wsgi.ini` 의 `touch-reload` 가 가리키는 파일을 touch 하여 워커 전체를 graceful reload 합니다. |
| 00:20 | `log_retention` | 로그 디렉터리(`/project/workSpace/logs`)에서 7일보다 오래된 로그 파일을 삭제합니다. |

재기동 대상 경로는 **`wsgi.ini` 한 곳에만** 적습니다. 작업이 그 줄을 직접 읽어
경로를 알아내므로, 경로를 바꿔도 코드 수정은 필요 없습니다. 반대로 그 줄이
없으면 작업은 아무것도 touch 하지 않고 그 사실을 로그에 남깁니다 — 감시하지도
...
로그 정리의 기준은 파일명의 날짜가 아니라 **mtime** 입니다. 하루치 로그는 그 날이
끝나면 더 쓰이지 않으므로 mtime 이 곧 그 날짜이고, 날짜가 이름에 없는 회전 파일
(`*.log.1`)도 같은 규칙으로 정리됩니다. 삭제 범위는 의도적으로 좁습니다 —
하위 디렉터리는 순회하지 않고, 심볼릭 링크는 따라가지 않으며,
`SKEWNONO_LOG_GLOB` 에 맞는 이름만 지웁니다. 보관 일수(`SKEWNONO_LOG_RETENTION_DAYS`)를
0 이나 음수로 두면 전부 지우는 대신 작업이 스스로 멈춥니다.

두 작업 모두 `GET /api/health/jobs` 에 다른 작업과 같은 방식으로 기록됩니다.
다만 `uwsgi_touch_reload` 의 `end` 기록은 재기동이 그 프로세스를 먼저 정리하면
남지 않을 수 있습니다. 이때의 확실한 기록은 uWSGI 로그의
`uwsgi_reload touching ...` 줄입니다.
...
사무실에서 확인할 항목으로 `docs/datatables/hitachi/members.txt` 에 남겨 두었습니다.
