...
**para_* 구간 버킷과의 관계**: device-statistics 의 `para_N` 컬럼(= point 수가 그 **구간**에 드는 파라미터 *개수*. 2026-08-10 부터 정확 일치가 아니라 구간이며, `para_over_16` 이 열린 위 구간입니다 — `backend/ebeam/device_statistics/para_buckets.py`)은 파라미터 단위 데이터에서 **파생되는 집계 view** 입니다. `para_N` bin 만으로는 `EDGE_EX=0`·`LEVEL=4` 같은 타입별 룰을 표현할 수 없어, raw 파라미터 데이터가 source of truth 입니다.
...
채팅의 거래 단위. 사용자 질문 하나와 그에 대한 답변 하나가 한 턴이며, 턴은
`request_id`(canonical UUID)로 식별됩니다 — 같은 UUID 로 다시 오면 새로 묻지
않고 이미 만든 답을 돌려줍니다(네트워크 재시도와 같은 문장의 새 질문을 가르는
것이 이 값입니다). 한 턴이 한 번의 모델 호출이 **아니라는** 점이 중요합니다:
질의 재작성, 검색, rerank, 후속 질문 생성이 각자 시간을 쓰므로 예산은 턴
전체에 대해 하나로 잡습니다.

턴은 **리소스**입니다 — 질문을 보내는 것과 답을 받는 것이 같은 왕복일 필요가
없습니다. 턴은 시작되는 순간 존재하고(아직 답이 없는 상태로), 답이 생기거나
실패로 끝납니다. 그래서 창을 닫았다 열어도 진행 중이던 턴을 이어서 볼 수
있고, 답을 만드는 동안 서버가 그 사용자만을 위해 붙잡혀 있지 않습니다.

### 답변 이음매 (Answer Seam)

이 앱과 사내 RAG 가 만나는 단 하나의 지점. 질문·직전 대화·[[access-scope]]·
예산을 넘기면 답변 본문과 [[evidence]] 목록이 돌아옵니다. 검색을 몇 번
반복할지, 어떤 모델로 답을 쓸지는 전부 이음매 **너머**의 일이며 이 앱은
알지 못합니다 — 그래서 이 앱에는 자체 LLM 호출이 없습니다.

이음매의 계약(어떤 필드가 필수인가, 무엇이 오류인가)은 산문이 아니라
양쪽이 실행하는 코드입니다. 한쪽이 계약을 어기면 조용한 성능 저하가 아니라
어긋난 필드 이름을 말하는 오류가 됩니다.

### 근거 (Evidence)

답변이 인용하는 원문 조각 한 건. 매뉴얼·회의록·메일·보고서 중 하나에서
왔으며, 사용자가 답을 믿을지 판단하는 재료입니다. 한 답변이 인용할 수 있는
근거 수의 상한은 **애플리케이션이 정합니다** — 모델도 RAG 도 늘릴 수
없습니다. 근거가 그림에서 추출된 것이면 불투명한 그림 토큰을 함께 갖고,
본문·표에서 왔으면 갖지 않습니다. 후자가 더 흔합니다.

### 접근 범위 (Access Scope)

질문한 사람이 볼 수 있는 문서의 범위. 검색 **질의 단계**에 들어가며, 검색한
뒤 걸러 내는 방식은 허용하지 않습니다 — 걸러 내기 전의 결과가 이미 사용자가
볼 수 없는 문서를 읽은 것이기 때문입니다.

### RAG 동거 (Co-location)

사내 RAG 를 별도 서비스로 두지 않고 이 앱과 같은 곳에 두고 직접 부르는 배치.
RAG 는 별도 저장소이며 이 저장소로 밀어 넣지 않습니다. 두 저장소가 한
프로세스를 공유한다는 뜻이므로, 의존성·수명·배포가 함께 묶입니다.
