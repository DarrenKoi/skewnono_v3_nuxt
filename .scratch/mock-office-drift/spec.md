...
| 단계 | 무엇을 재는가 | 걸린 이슈 |
| --- | --- | --- |
| 1 | `msr_check` 어휘(`Yes` vs `Y`)와 `meastime` 결측률 | 04 |
| 2 | `fail_ratio` 결측률, 두 분모의 실제 차이 | 03 |
| 3 | 두 lot_cd 카탈로그의 교집합 | 01, 02 |
| 4 | `worst drift_sigma` 분포 (MSR 표본) | 05 |
| 5 | 빈 `mp_image_name 01` 관측 여부 (같은 표본) | 07 |

1단계는 덤으로 **`msr_check` 값이 정말 `"Yes"` 인지**도 봅니다 — 인덱스가
`"Y"` 를 저장한다면 `office_example.py` 의 `.lower() == "yes"` 가 모든 행을
`"No"` 로 뒤집으면서 오류 하나 내지 않습니다.

4·5단계는 MinIO 에서 pickle 을 읽으므로 느립니다. `--msr-sample 0` 으로
건너뛸 수 있고, 한 단계가 실패해도 나머지는 계속 돕니다.
