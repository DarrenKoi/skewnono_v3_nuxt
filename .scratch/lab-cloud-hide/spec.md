...
> **상태: 폐기(superseded) — 2026-09-01, `e8c78797`.** TTTM 이 프로덕션 클라우드에
> 정식 공개되면서 아래 요구사항 1·3·6 은 더 이상 유효하지 않습니다. 여기서 만든
> `hiddenOnCloud` 행 플래그, `visibleHeaderLinksIn()`, `useDeployment.ts`,
> `GET /api/health/deployment` 는 모두 삭제되었습니다. 다시 숨겨야 할 페이지가
> 생기면 이 문서는 그때의 판단 기록으로 읽되(요구사항 2·4·5 는 그대로 유효합니다),
> 구현은 새로 시작해야 합니다 — `docs/adr/0006-page-grouping-by-domain-and-object.md`
> 의 "결과로 따라오는 제약" 항목이 되살릴 자리를 가리킵니다.
