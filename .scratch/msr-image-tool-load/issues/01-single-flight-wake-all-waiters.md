...
요구했던 "지금 코드에 대해 실패해야 하는" 테스트는 **쓸 수 없습니다.** handoff
버전은 `with` 블록을 받았으므로 새 시그니처로 쓴 테스트를 그 위에서 돌릴 방법이
없고, 처음 작성한 타이밍 테스트는 대기자의 작업을 프리미티브 **밖** 에 두어
옛 코드에서도 통과했을 것입니다(그 상태로는 아무것도 지키지 못했습니다).

대신 `test_the_waiters_are_all_released_at_once` 가 성질을 직접 고정합니다 —
대기자 3명이 `Barrier(3, timeout=0.5)` 에 함께 도달하지 못하면 실패합니다.
회귀 테스트가 아니라 성질 테스트라는 점을 테스트 docstring 에 적어 두었습니다.
