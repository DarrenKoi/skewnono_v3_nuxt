...
최종 리뷰가 제품 판단으로 남긴 2건을 사용자 확인 후 **둘 다 고쳤습니다.**
두 건은 같은 성질이었습니다 — 파일 하나 안에서 서로 다른 사실이 나란히
놓이는 자리입니다.

| 항목 | 이전 | 이후 |
| --- | --- | --- |
| 검색으로 걸러진 선택 장비 | `장비` 시트에서만 빠져 세 시트가 다른 집합을 말함 | `exportEquipmentRows()`가 보이는 행 뒤에 덧붙임 |
| `exec_count`가 0인 행의 파생값 | `장비` 시트는 `0`, `레시피` 시트는 빈 칸 | 두 시트 모두 빈 칸 |

클립보드 복사는 보정하지 않습니다 — 표 하나를 그대로 옮기는 용도라 어긋날
두 번째 시트가 없습니다.

"돌지 않은 것"과 "돌았는데 0인 것"은 다른 문장이라는 규칙이 이제 시트를
가리지 않습니다. 화면은 실행수 0을 옆 칸에 함께 보여주지만, 파일에서는 그
문맥이 열 하나 건너에 있습니다.
