...
- **Dense exception (1440px):** list-plus-detail pages may widen one step to `max-w-[1440px]`. Current members: H/W management (`ebeam/HardwareView.vue`, 320px list rail + `1fr` detail) and Mag/Pixel 가이드 (`pages/mag-pixel.vue`, 392px sticky input-and-answer rail + `1fr` drawings and reference table). The two lab pages — 장비간 스큐 관리 (`ebeam/TttmView.vue`) and PM 튜닝 (`ebeam/PmTuneView.vue`) — keep the same width on the **scope-bar** layout below rather than the rail one. Device Statistics and Time-Series are candidates. Agreed pattern — do not revert.
  ...
  - **The scope-bar rule (2026-08-18):** where the analysis is meaningless until the user has *said what to analyse*, the controls stop being a rail and become one full-width bar above the results — `EbeamScopeBar`, titled **비교 대상**. The rail rule's substance is unchanged (controls above, results below, never mixed); what changes is the axis, and the reason is reading order. A rail puts the first decision to the side of a screen already full of numbers; a bar puts it first, which is where a required decision belongs. Two conditions make this the right choice over a rail, and both must hold:
    - **The results are gated.** Until the scope is set, the results area carries an explicit `AppEmptyState` naming the missing choice — never a skeleton (which promises data that is not coming) and never a zeroed card (which reads as a computed verdict). TTTM and PM 튜닝 gate on the **recipe** alone; the parameter stays optional because its list is resolved through recipe-open over FTP, and a required field behind a failable request is a page that can lock shut.
    - **The scope is shared or small.** One bar row holds roughly three control cells. TTTM and PM 튜닝 hold 장비·모델 그룹 + RECIPE + PARAMETER, with the page-specific control (TTTM's tolerance knob, PM 튜닝's 튜닝할 장비) in a divided trailing cell — divided because a threshold and a subject are not part of *what is being compared*. A scope needing more cells than that wants a rail.
