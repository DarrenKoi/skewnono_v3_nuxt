...
- **Dense exception (1440px):** list-plus-detail pages may widen one step to `max-w-[1440px]`. Current members: H/W management (`ebeam/HardwareView.vue`, 320px list rail + `1fr` detail), Mag/Pixel 가이드 (`pages/mag-pixel.vue`, 392px sticky input-and-answer rail + `1fr` drawings and reference table) and 장비간 스큐 관리 (`ebeam/TttmView.vue`, 392px sticky control rail + `1fr` results). Device Statistics and Time-Series are candidates. Agreed pattern — do not revert.
  - **The rail rule:** the rail holds only controls and the roll-up of what they cost; the results column holds no control at all. That split is why the width is granted — a page that puts a filter back into the results column has stopped being list-plus-detail and should go back to one column.
