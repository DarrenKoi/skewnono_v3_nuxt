...
- **Dense exception (1440px):** list-plus-detail pages may widen one step to `max-w-[1440px]`. Current member: Mag/Pixel 가이드 (`pages/mag-pixel.vue`, 392px sticky input-and-answer rail + `1fr` drawings and reference table). The two lab pages — 장비간 스큐 관리 (`ebeam/TttmView.vue`) and PM 튜닝 (`ebeam/PmTuneView.vue`) — and H/W 관리 (`ebeam/HardwareView.vue`, since 2026-08-25) keep the same width on the **scope-bar** layout below rather than the rail one. Device Statistics and Time-Series are candidates. Agreed pattern — do not revert.
