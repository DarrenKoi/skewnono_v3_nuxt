...
- **Dense exception (1440px):** list-plus-detail pages may widen one step to `max-w-[1440px]`. Current member: Mag/Pixel 가이드 (`pages/mag-pixel.vue`, 392px sticky input-and-answer rail + `1fr` drawings and reference table). The two lab pages — 장비간 스큐 관리 (`ebeam/TttmView.vue`) and PM 플래닝 (`ebeam/PmPlanningView.vue`) — keep the same width on the **scope-bar** layout below rather than the rail one; H/W 관리 (`ebeam/HardwareView.vue`, since 2026-08-25) runs that layout's *model-gated* strip, named inside the rule. Device Statistics and Time-Series are candidates. Agreed pattern — do not revert.
  ...
  - **The scope-bar rule (2026-08-18):** where the analysis is meaningless until the user has *said what to analyse*, the controls stop being a rail and become one full-width bar above the results — `EbeamScopeBar`, titled **비교 대상**. The rail rule's substance is unchanged (controls above, results below, never mixed); what changes is the axis, and the reason is reading order. A rail puts the first decision to the side of a screen already full of numbers; a bar puts it first, which is where a required decision belongs. Two conditions make this the right choice over a rail, and both must hold:
    ...
    - **The subject of the page is its own bar, and it follows the chip that summons it (2026-08-27; moved below 분석 조건 2026-09-01).** 튜닝할 장비 (`ebeam/pmPlanning/ToolPicker.vue`) left 분석 조건's trailing cell for a full-width bar of its own, and since the two lab pages merged into one it sits **directly below 분석 조건**. The other three bars are permanent and say what data to *compute from*; this one names the single tool the page reports on — every target cell, the Up gate card and the ranking below are computed for it — and it is not always on the page at all: the **PM 튜닝 chip inside 분석 조건 is what summons it**, so the bar belongs directly under the chip that conjures it, not somewhere above it where a click makes a bar appear off the reader's eye. The ordering that first moved it here still holds either way — 비교 대상 and 장비 모델 그룹 are both above, so the tool is still picked out of a set that has already been defined. A 264px cell at the far right of 분석 조건, under a title that did not name it, put the page's most consequential pick where it was hardest to see, which is why it is a bar and not a cell. It is **never locked**: the PM roster is its own request and does not ride on the recipe, so picking the tool before the scope is a legitimate order and a greyed-out control would read as broken — the 그룹 badge simply does not appear until there is a group to be in. The picked id renders at `.sk-card-id` size with its Up gate / 1차 그룹 / last-PM facts beside the trigger, because a collapsed trigger can only carry an id; the 튜닝 목표 card repeats that id and its `eqp_model_cd` in the same styling, since every row of that table is a distance measured for the one tool.
