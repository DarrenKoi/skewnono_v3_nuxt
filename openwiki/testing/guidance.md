...
- Skewvoir: `app/utils/skewvoirAnalysis/*.test.ts`, including `variantMemory.test.ts` for recipe/parameter-scoped sub-image suffix memory, `app/utils/anomaly/*.test.ts`, `overview.test.ts`
...
- Recipe-status analytics and exports: `recipeStatusTrend.test.ts`, `recipeStatusSummary.test.ts`, `csvDownload.test.ts`, and `equipmentExport.test.ts`; also run `parameterRamp.test.ts` for theme-linked Device Statistics ramps. The six equipment Vue components currently have no component-specific automated tests, so verify line/bar switching, empty-state control disabling, filtered/sorted fleet rows, complete recipe-union export, active Align/Meas scoping, and no-execution blank rates in a browser check.
...
| --- | --- |
| Flask app/auth/logging | App startup; `_auth` identity-chain, identify/verify/directory/admin tests; shared `50 per 5 seconds` rate-limit boundary, anonymous-bucket, and `msr_image`-exemption regressions; session/proxy and error-handler regressions; identity input/display utilities; `_logging` middleware/handler/target suites; logging health route, activity/admin-log contracts, alias-provisioning guardrails, identity-source persistence, nonblocking drops, and endpoint-specific 503 behavior |
...
| Recipe-status equipment analytics | Shared CSV/clipboard serializer, `equipmentExport.test.ts`, and `parameterRamp.test.ts`; browser verification of grouped line/bar trends, theme-linked parameter colors, filtered/sorted fleet export, complete recipe-union matrix export, active Align/Meas fail export, peer-signal suppression, empty-state disabling, and blank rates for equipment with no executions |
...
| Skewvoir/MSR images | Meas-hist percentage and `eqp_ip` contracts, MSR detail and geometry round-trip tests, cursor/composite-selection/site-color/parameter-matrix utilities, FDC matrix/individual graph-selection reconciliation and parameter/all-axis restoration, chip/coordinate/mean correlation pairing, recipe/parameter-scoped variant-memory tests, gallery scale/failure and no-download-all UI checks, image route/cache/memory+Redis job/FTP-template/scheduler/app-wiring suites, image-kind and wafer geometry/grid/axis utilities |
