...
- Recipe/hardware/live feeds: `recipeSearchMatch.test.ts`, `recipeSelection.test.ts`, `recipeView.test.ts`, `hardwareCompare.test.ts`, `fdcValues.test.ts`, `liveAlarm.test.ts`, `useLiveAlarmFeed.test.ts`
...
| --- | --- |
| Flask app/auth/logging | App startup, API-token/access tests, `_logging` policy/middleware/handler/target suites, logging health route, activity/admin-log contracts, alias-provisioning guardrails, nonblocking drops and endpoint-specific 503 behavior |
...
| Recipe/hardware operations | Recipe ranking/complete-fallback/source-capability and IDP-mapping tests, BM/PM and BSM office mappings, MDC/Reso Center reconstruction contracts, search-scoped hardware selection, SCE history, FDC parser tests, lateral measured-implies-ready consistency, representative office diagnosis |
...
| Skewvoir/MSR images | Meas-hist percentage and `eqp_ip` contracts, MSR detail and geometry round-trip tests, cursor/composite-selection/site-color/parameter-matrix utilities, FDC parameter/all-axis alignment and URL restoration, gallery scale and failure checks, image route/cache/memory+Redis job/FTP-template/scheduler/app-wiring suites, image-kind and wafer geometry/grid/axis utilities |
...
`ops_store`, `minio_handler`, and `ftp_handler` have no clear package-local test suites in the inspected tree. `ops_index_mgmt` now has characterization and guardrail coverage in `tests/test_vendored_ops_index_mgmt.py`, including logging-family aliases, mappings, lifecycle policy, and runtime-target agreement. Browser-level tests remain important for the URL-heavy analytical workflows and for admin-log stale-row suppression, recipe fallback actions, FDC axis restoration, and hardware picker keyboard behavior.
