...
- **First cloud deployment** — `docs/deployment.md` and `scripts/deploy/`: packaging and preflight are implemented and locally tested; build with `python -m scripts.deploy`, then perform the first `/project/workSpace` feasibility deployment, SSO registration, permission restoration, and post-start provider check.
