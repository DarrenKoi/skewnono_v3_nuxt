...
- **Live alarm source verification** — `back_dev_home/ebeam/live_alarm/MIGRATION.md` and `contracts.py`: verify the office `get_ebeam_metrology_alarms(fac_id)` timeout/schema against representative facilities and identify the still-unknown AMAT measurement-failure ALIDs; the Hitachi cached-pull, multi-FAB board, and provider swap are implemented.
- **Rule persistence** — `back_dev_home/ebeam/device_statistics/` and `useMeasurementRulesApi.ts`: implement save, history, rollback, and identity attribution after the datastore is chosen.
- **Measurement image office activation** — `back_dev_home/msr_image/MIGRATION.md`: the FTP adapter, multi-image gallery, inline TIFF-to-WebP previews with original download, path guards, bounded jobs, and optional Redis state are implemented; create the ignored office copy, configure an isolated MinIO prefix and Redis, align the external sweep to the 72-hour default, then verify representative FTP/cache behavior.
...
Recent update scope: the shared API budget is now 50 requests per 5 seconds with per-user/IP bucketing and continued `msr_image` exemption; Skewvoir remembers SEM sub-image picks per recipe and parameter; equipment analytics adds theme-linked parameter ramps and workbook export consistency; recipe IDP office normalization accepts only exact binary boolean sentinels; and deployment packaging supports explicit roots and foreign-CWD diagnostics. See [key workflows](workflows/key-workflows.md), [operations](operations/runbook.md), and [testing guidance](testing/guidance.md) for the affected details.
