...
description: End-to-end paths for Device Statistics, pending-tool firewall requests, recipe and hardware operations, live alarms, Skewvoir, AFM, chat, and provider-backed feature migration.
...
The landing page's System Status entry opens `/tool-roster`, not a permanent top-navigation item. `GET /api/sem-list/pending` computes the company roster minus the SKEWNONO-reachable fleet by `eqp_id`; it stays separate from `/api/sem-list` so unreachable tools do not appear in ordinary equipment selectors. The page removes exact loopback placeholders, classifies CD-SEM, HV-SEM, VeritySEM, Provision, and unknown future models, preserves blank fabs under `미배정`, and builds fab-by-model counts with drill-down rows. Engineers can copy distinct IPs or export the active tool-type scope for IT firewall requests. Route re-entry reuses the Nuxt session cache; explicit refresh refetches (`sem_list/providers/office_example.py`, `usePendingToolsApi.ts`, `pendingToolMatrix.ts`).
