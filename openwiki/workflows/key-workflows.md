...
`useMeasHistSearch.ts` and `useMeasHistFacets.ts` drive discovery. `useSkewvoirRoute.ts` owns URL serialization, while `useSkewvoirAnalysis.ts` fetches and caches focus/set data with stale-response protection. FDC analysis is intentionally a single-MSR, sequence-order view; Time-Series is the multi-MSR/set trend view. The default FDC axis contains only sequences for the active parameter, while `fdcaxis=all` opts into the whole-MSR union. Failed measurement rows remain visible as CD gaps with aligned FDC state, and row/`dynamic_fdc` count mismatches surface as integrity warnings rather than being hidden.
...
tion startup because discovery imports every route module. Prefer a clear `NotImplementedError` over an office adapter returning plausible empty data.
