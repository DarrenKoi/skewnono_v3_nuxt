...
- Home 환경은 `backend/meas_hist/data.py`의 메모리 mock을 사용하며 OpenSearch에 연결하지 않습니다.
...
2. ingest 시 `backend.meas_hist.opensearch_query.build_search_all_value()` 결과를 `search_all`에 저장합니다.
